...
bool EPD_WaitUntilIdle(uint8_t timeout = 100, uint8_t min = 0) {
  ...
  while (digitalRead(PIN_SPI_BUSY) == 0) {
    if (t >= timeout) {
      Serial.println("[ERROR] WaitUntilIdle Timeout!");
      return false;
    }
    delay(100);
    t++;
  ...
  if (t < min && min != 0) {
    Serial.println("[ERROR] WaitUntilIdle idled too fast!");
    return false;
  }
  if (t == 0) {
    Serial.println("[WARN] WaitUntilIdle never became busy!");
    return true;
  }
  return true;
}
...
bool EPD_WaitUntilIdle_high(uint8_t timeout = 100, uint8_t min = 0) {
  ...
  while (digitalRead(PIN_SPI_BUSY) != 0) {
    if (t >= timeout) {
      Serial.println("[ERROR] WaitUntilIdle_high Timeout!");
      return false;
    }
    delay(100);
    t++;
  ...
  if (t == 0) {
    Serial.println("[WARN] WaitUntilIdle_high never became busy!");
    return true;
  }
  if (t < min) {
    Serial.println("[ERROR] WaitUntilIdle_high idled too fast!");
    return false;
  }
  return true;
}
...
void EPD_loadB() {
  ...
  while (pos < Buff_msgIndex) {
    ...
    int valueA = (int)Buff_getByte(pos) + ((int)Buff_getByte(pos + 2) << 8);
...
struct EPD_dispInfo {
  ...
  void (*loadBlk)();  // Black channel loading
  int next;           // Change channel code
  void (*loadRed)();  // Red channel loading
  bool (*show)();     // Show and sleep
