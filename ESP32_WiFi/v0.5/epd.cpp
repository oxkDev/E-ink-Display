...
SPISettings epdSpiSettings(20000000, MSBFIRST, SPI_MODE0);
...
void EPD_initSPI() {
  ...
  epdSpi.begin(PIN_SPI_SCK, -1, PIN_SPI_DIN, -1);
}
...
/* Waiting the e-Paper is ready for further instructions ---------------------*/
bool EPD_WaitUntilIdle_high(uint8_t timeout, uint8_t min) {
  //1: busy, 0: idle
  uint8_t t = 0;
  while (digitalRead(PIN_SPI_BUSY) != 0) {
    if (t >= timeout) {
...
void EPD_loadB() {
  ...
  while (pos < Buff_msgLength) {
    ...
    uint16_t valueA = (int)Buff_getByte(pos) + ((int)Buff_getByte(pos + 2) << 8);
