...
const palettes = [
    [[0, 0, 0], [255, 255, 255]], // B W
    [[0, 0, 0], [255, 255, 255], [127, 0, 0]], // B W R
    [[0, 0, 0], [255, 255, 255], [127, 127, 127]], // B W Gry
    [[0, 0, 0], [255, 255, 255], [127, 127, 127], [127, 0, 0]], // B W Gry R
    [[0, 0, 0], [255, 255, 255]], // B W (2.13d, 2.9d)
    [[0, 0, 0], [255, 255, 255], [220, 180, 0]], // B W Y
...
function setPalette(paletteIndex) {
    if (paletteIndex != curPalette.index) {
        console.log("Palette changed, clearing RGB cache.");
        rgbDistCache.clear();
        curPalette.index = paletteIndex;
    }
    curPalette.rgb = palettes[paletteIndex];
    curPalette.lab = curPalette.rgb.map(c => rgbToLab(c[0], c[1], c[2]));

    if (paletteIndex < 6) {
        curPalette.codes = [0, 1, 2, 3];
        return;
    }
...
let epdIndex = 50;
let curPalette = {
    index: epdData[epdIndex].paletteIndex,
    rgb: [],
    lab: [],
    codes: []
};
let rgbDepth = 0xFF;
let rgbDistCache = new Map();
...
function processInputs(resetCache = false) {
    if (resetCache) {
        console.log("Clearing RGB cache.");
        rgbDistCache.clear();
        rgbDistCache.clear();
    }
...
window.addEventListener("load", () => {
    ...
    const inputIDs = ["bgColour", "setColour", "setDithering", "rotateMode", "scaleMode", "imgH", "imgW", "imgY", "imgX", "quality"];

    for (let i = 0; i < inputIDs.length; i++)
        getElm(inputIDs[i]).addEventListener("change", () => processInputs(inputIDs[i] == "quality"));
...
function setColourCode(pixelIndex, colourIndex) {
    pixelIndex *= 4;
    dstImgData.data[pixelIndex] = curPalette.rgb[colourIndex][0]; // R
    dstImgData.data[pixelIndex + 1] = curPalette.rgb[colourIndex][1]; // G
    dstImgData.data[pixelIndex + 2] = curPalette.rgb[colourIndex][2]; // B
    dstImgData.data[pixelIndex + 3] = 255; // A

    dstColourCodesBuffer[pixelIndex] = curPalette.codes[colourIndex];
}
...
function getNear(r, g, b) {
    ...
    let key = ((r & rgbDepth) << 16) | ((g & rgbDepth) << 8) | (b & rgbDepth);
    ...
    while (i < curPalette.lab.length) {
        let p = curPalette.lab[i];
...
function processImg() {
    ...
    rgbDepth = parseInt(getElm("quality").value);
    console.log(`Process colour depth: ${rgbDepth.toString(2)}`);
    ...
    if (!dithering) {
        ...
        for (let row = 0; row < imgH; row++) {
            ...
            for (let col = 0; col < imgW; col++) {
                ...
                let srcRGB = [0, 0, 0];
                // if (col >= srcW || (row >= srcH)) {
                //     srcRGB = bgRGB;
                // } else {
                let pos = pixelIndex * 4;
                srcRGB = [
                    srcImgData.data[pos],
                ...
                let targetRGB = [0, 0, 0];
                for (let k = 0; k < 3; k++) {
                    targetRGB[k] = Math.max(0, Math.min(255, srcRGB[k] + errBuffers[errRowA][col][k]));
                }

                let colourIndex = getNear(targetRGB[0], targetRGB[1], targetRGB[2]);

                setColourCode(pixelIndex++, colourIndex);

                let errRGB = [
                    targetRGB[0] - curPalette.rgb[colourIndex][0],
                    targetRGB[1] - curPalette.rgb[colourIndex][1],
                    targetRGB[2] - curPalette.rgb[colourIndex][2]
                ];
